...
namespace k5tool
{
    ...
    public class Source
    {
        ...
        public Source(byte[] data)
        {
            ...
            (b, offset) = Util.GetNextByte(data, offset);
            EnvelopeDepth = (sbyte)b;

            (b, offset) = Util.GetNextByte(data, offset);
            PressureDepth = (sbyte)b;

            (b, offset) = Util.GetNextByte(data, offset);
            BenderDepth = b;

            (b, offset) = Util.GetNextByte(data, offset);
            VelocityEnvelopeDepth = (sbyte)b;

            (b, offset) = Util.GetNextByte(data, offset);
            LFODepth = b;

            (b, offset) = Util.GetNextByte(data, offset);
            PressureLFODepth = (sbyte)b;
            ...
            for (int i = 0; i < PitchEnvelopeSegmentCount; i++)
            {
                ...
                (b, offset) = Util.GetNextByte(data, offset);
                PitchEnvelope.Segments[i].Level = (sbyte)b;
            }
	        ...
	        (b, offset) = Util.GetNextByte(data, offset);
        	harmSet.VelocityDepth = (sbyte)b;

	        (b, offset) = Util.GetNextByte(data, offset);
        	harmSet.PressureDepth = (sbyte)b;

            (b, offset) = Util.GetNextByte(data, offset);
        	harmSet.KeyScalingDepth = (sbyte)b;
